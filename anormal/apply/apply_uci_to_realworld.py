...
with torch.no_grad():
    ...
    for i in range(0, abnormal_data.size()[0], batch_size):
        ...
        # 输出
        if show_count < 5:
            show_tensor_data(input_data, outputs, loss, dataset_name, title='realworld-normal-showcase')
            show_count += 1
...
with torch.no_grad():
    ...
    for i in range(0, normal_data.size()[0], batch_size):
        ...
        # 输出
        if show_count < 5:
            show_tensor_data(input_data, outputs, loss, dataset_name, title='realworld-abnormal-showcase')
            show_count += 1
