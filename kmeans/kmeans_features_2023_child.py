...
for d in origin_data:
    ...
    aex,aey,aez,aet = calc_fft_spectral_energy(d.iloc[:, :9], acc_x_name='accx', acc_y_name='accy', acc_z_name='accz', T=simpling)
    gex,gey,gez,get = calc_fft_spectral_energy(d.iloc[:, :9], acc_x_name='angx', acc_y_name='angy', acc_z_name='angz', T=simpling)
    mex,mey,mez,met = calc_fft_spectral_energy(d.iloc[:, :9], acc_x_name='magx', acc_y_name='magy', acc_z_name='magz', T=simpling)
    df_features['fft_spectral_energy'] = [aex,aey,aez,gex,gey,gez,mex,mey,mez]

    # 分别对9维数据XYZ求FFT的能量(结果会变坏)
    aex,aey,aez,aet = spectral_entropy(d.iloc[:, :9], acc_x_name='accx', acc_y_name='accy', acc_z_name='accz', T=simpling)
    gex,gey,gez,get = spectral_entropy(d.iloc[:, :9], acc_x_name='angx', acc_y_name='angy', acc_z_name='angz', T=simpling)
    mex,mey,mez,met = spectral_entropy(d.iloc[:, :9], acc_x_name='magx', acc_y_name='magy', acc_z_name='magz', T=simpling)
